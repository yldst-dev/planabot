...
- 2026-09-08: 응답 지연 개선. 게이트웨이 요청 기록으로 재보니 Gemini 웹의 3.8 Flash는 한 문장 답변에도 8초에서 90초가 걸리고 편차가 크며, 3.5 Flash-Lite는 4초에서 5초다. 게이트웨이(v4.15.0) 소스상 일반 모델도 thinking 값 1로 보내고 끄는 값은 없으며, `-thinking` 접미 모델만 확장 사고를 켠다. 스트림 여부는 총 시간에 영향이 없다. 그래서 검색어 재작성과 전달문 재작성 같은 보조 호출을 `PLANABRAIN_AUX_PROVIDER`, `PLANABRAIN_AUX_MODEL`로 빠른 제공자에 보내도록 했고(자격 증명이 없으면 주 제공자로 복귀), 링크 문맥 수집과 검색어 재작성·사전 검색을 병렬로 돌리며, 턴마다 준비·답변·전달 단계 시간을 표준 오류에 남긴다. 로컬에서는 보조 호출을 ollama.com의 gemma4:31b(약 0.75초)로 보내 날씨 검색 턴의 준비 시간이 약 10초에서 1.5초로 줄었다. 남은 시간은 거의 전부 Gemini 웹 모델의 생성 시간이며 주 모델을 `gemini-3.5-flash-lite`로 바꾸면 4초에서 5초 안에 끝난다. 게이트웨이 쪽 추가 여지: XSRF 캐시가 쿠키 문자열 해시로 키를 잡아 거의 매 요청마다 `/app` 페이지를 다시 받는다(요청당 0.3초에서 1.5초). `.env`의 Cerebras 키는 현재 유효하지 않다.
